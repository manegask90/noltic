...
class SQ_Models_Focuspages_Strategy extends SQ_Models_Abstract_Assistant {
    ...
    public function setTasks($tasks) {
        ...
        $this->_tasks[$this->_category] = array(
            ...
            'labels' => array(
                ...
                'value' => ($this->_labels ?  __('Great! The keyword has Label attached to it', _SQ_PLUGIN_NAME_) : __('The keyword does not have a label attached to it', _SQ_PLUGIN_NAME_)),
    ...
    public function getHeader() {
        ...
        if ($this->_keyword) {
            ...
            $header .= '<a href="' . SQ_Classes_Helpers_Tools::getAdminUrl('sq_research', 'briefcase') . '" target="_blank" class="btn btn-success text-white col-sm-8 offset-2 mt-3">' . __('Manage Strategy', _SQ_PLUGIN_NAME_) . '</a>';
        } else {
    ...
    public function checkLsioptimization($task) {
        if (isset($this->_audit->data->sq_seo_briefcase) && !empty($this->_audit->data->sq_seo_briefcase)) {
            ...
            $task['error_message'] = __('Add a secondary keyword in Squirrly Live Assistant from Briefcase', _SQ_PLUGIN_NAME_);
