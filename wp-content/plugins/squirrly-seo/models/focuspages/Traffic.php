...
class SQ_Models_Focuspages_Traffic extends SQ_Models_Abstract_Assistant {
    ...
    public function getHeader() {
        ...
        if (!$this->_audit->sq_analytics_gsc_connected) {
            $header .= '<li class="completed">
                    <a href="' . SQ_Classes_RemoteController::getApiLink('gscoauth') . '"  target="_blank" class="btn btn-success text-white col-sm-10 offset-1 mt-1">' . __('Connect Google Search', _SQ_PLUGIN_NAME_) . '</a>
                </li>';
        }
        if (!$this->_audit->sq_analytics_google_connected) {
            $header .= '<li class="completed">
                    <a href="' . SQ_Classes_RemoteController::getApiLink('gaoauth') . '"  target="_blank" class="btn btn-success text-white col-sm-10 offset-1 mt-1">' . __('Connect Google Analytics', _SQ_PLUGIN_NAME_) . '</a>
                </li>';
        }
        return $header;
    }
