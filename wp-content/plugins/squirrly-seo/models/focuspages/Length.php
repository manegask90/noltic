...
class SQ_Models_Focuspages_Length extends SQ_Models_Abstract_Assistant {
    ...
    public function getHeader() {
        ...
        if (isset($this->_post->ID)) {
            ...
            $header .= '<li class="completed">';
            if (isset($this->_post->ID)) {
                $header .= '<a href="' . $edit_link . '" target="_blank" class="btn btn-success text-white col-sm-8 offset-2 my-2">' . __('Edit Page', _SQ_PLUGIN_NAME_) . '</a>';
            }
            if (!$this->_audit->sq_analytics_google_connected) {
                $header .= '<a href="' . SQ_Classes_RemoteController::getApiLink('gaoauth') . '"  target="_blank" class="btn btn-success text-white col-sm-10 offset-1 mt-1">' . __('Connect Google Analytics', _SQ_PLUGIN_NAME_) . '</a>';
            }
            $header .= '</li>';
