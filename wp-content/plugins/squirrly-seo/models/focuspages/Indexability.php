...
class SQ_Models_Focuspages_Indexability extends SQ_Models_Abstract_Assistant {
    ...
    public function getHeader() {
        ...
        if (!$this->_audit->sq_analytics_gsc_connected && !$this->_audit->sq_analytics_google_connected) {
            $header .= '<li class="completed">
                    <a href="' . SQ_Classes_RemoteController::getApiLink('gscoauth') . '"  target="_blank" class="btn btn-success text-white col-sm-10 offset-1 mt-3">' . __('Connect Google Search', _SQ_PLUGIN_NAME_) . '</a>
                </li>';
        } else {
            $header .= '<li class="completed sq_save_ajax">
                        <input type="hidden" id="sq_indexability_completed" value="1"/>
                        <button type="button" class="btn btn-success text-white col-sm-10 offset-1 mt-1" data-redirect="https://search.google.com/search-console/inspect" data-input="sq_indexability_completed" data-action="sq_ajax_assistant" data-name="indexability|gscindex|' . $this->_post->ID . '" >' . __('Ask Google to Re-Index', _SQ_PLUGIN_NAME_) . '</button>
                    </li>';
        }
