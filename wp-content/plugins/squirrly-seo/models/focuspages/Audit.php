...
class SQ_Models_Focuspages_Audit extends SQ_Models_Abstract_Assistant {
    ...
    public function setTasks($tasks) {
        ...
        $this->_tasks[$this->_category] = array(
            ...
            'description' => array(
                ...
                'description' => sprintf(__("Make sure that you do not have pages with empty descriptions. %s This means: pages where you haven't placed a meta description. %s Features like SEO Automation or SEO Snippet from Squirrly SEO will help you easily fix this problem by either automating or customizing descriptions for your pages.", _SQ_PLUGIN_NAME_), '<br /><br />', '<br /><br />'),
            ),
            'speed' => array(
    ...
    public function getHeader() {
        ...
        if ($this->_siteaudit && isset($this->_siteaudit->score)) {
            $header .= '<li class="completed">
                    <a href="' . SQ_Classes_RemoteController::getMySquirrlyLink('audits') . '" target="_blank" class="btn btn-success text-white col-sm-8 offset-2 mt-3">' . __('Go to Audit', _SQ_PLUGIN_NAME_) . '</a>
                </li>';
        } else {
            $header .= '<li class="completed">
                    <div class="font-weight-bold text-warning text-center">' . __('Note! The audit is not ready yet', _SQ_PLUGIN_NAME_) . '</div>
                     <a href="' . SQ_Classes_Helpers_Tools::getAdminUrl('sq_audits') . '" target="_blank" class="btn btn-success text-white col-sm-10 offset-1 mt-3">' . __('Request a new audit', _SQ_PLUGIN_NAME_) . '</a>
                    </li>';
        }
        ...
        if ($this->_duplicate_titles) {
            if (isset($this->duplicate_titles->urls) && !empty($this->_duplicate_titles->urls)) {
                ...
                foreach ($this->_duplicate_titles->urls as $url) {
                    $task['value'] .= __('URL', _SQ_PLUGIN_NAME_) . ': ' . $url . '<br />';
                }
            }
