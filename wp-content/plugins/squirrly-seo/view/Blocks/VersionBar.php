...
<div id="sq_notification_bar" style="margin: 20px 0 -15px 0;">
    ...
    <?php if (isset($view->checkin) && isset($view->checkin->subscription_status)) { ?>
        ...
        <?php if ($page == 'sq_focuspages' && isset($view->checkin->subscription_max_focus_pages)) { ?>
            <?php if ($view->checkin->subscription_max_focus_pages && $view->checkin->subscription_status == 'active') { ?>
                <div class="alert alert-success text-center m-0 p-1 small">
                    <?php echo sprintf(__("%sFocus Pages %s:%s Add maximum %s page(s) in Focus Pages and request a new audit for each page every 5 mins.", _SQ_PLUGIN_NAME_), '<strong>', $view->checkin->product_type, '</strong>', $view->checkin->subscription_max_focus_pages); ?>
                </div>
        ...
        <?php if ($page == 'sq_assistant') { ?>
            <?php if ($view->checkin->subscription_status == 'active') { ?>
                <div class="alert alert-success text-center m-0 p-1 small">
                    <?php echo sprintf(__("%sLive Assistant %s:%s Use Squirrly Live Assistant with all the optimization tasks to get 100%% optimized posts and pages.", _SQ_PLUGIN_NAME_), '<strong>', $view->checkin->product_type, '</strong>'); ?>
                </div>
            <?php } elseif ($view->checkin->subscription_status == 'freemium') { ?>
                <div class="alert alert-warning text-center m-0 p-1 small">
                    <?php echo sprintf(__("%sLive Assistant %s:%s Use the main SEO tasks to optimize your posts and pages. %sTo optimize your posts to 100%% please upgrade your plan to %sPRO Plan%s", _SQ_PLUGIN_NAME_), '<strong>', $view->checkin->product_type, '</strong>', '<br />', '<a href="' . SQ_Classes_RemoteController::getMySquirrlyLink('plans') . '" target="_blank"><strong>', '</strong></a>'); ?>
                </div>
            <?php } ?>
        <?php } ?>
        ...
        <?php if ($page == 'sq_research' && SQ_Classes_Helpers_Tools::getValue('tab', 'research') == 'research' && isset($view->checkin->subscription_kr) && isset($view->checkin->subscription_research)) { ?>
            <?php if ($view->checkin->subscription_status == 'active' && $view->checkin->subscription_research == 'light') { ?>
                <div class="alert alert-warning  text-center m-0 p-1 small">
                    <?php echo sprintf(__("%sResearch %s:%s You have %s researches left for your account. The research will return up to 20 results for each keyword. %sFor more Researches and up to 50 results per research, please upgrade your plan to %sBusiness Plan%s", _SQ_PLUGIN_NAME_), '<strong>', $view->checkin->product_type, '</strong>', (int)$view->checkin->subscription_kr, '<br />', '<a href="' . SQ_Classes_RemoteController::getMySquirrlyLink('plans') . '" target="_blank"><strong>', '</strong></a>'); ?>
                </div>
            <?php } elseif ($view->checkin->subscription_status == 'active' && $view->checkin->subscription_research == 'deep') { ?>
                <div class="alert alert-success text-center m-0 p-1 small">
                    <?php echo sprintf(__("%sResearch %s:%s You have %s researches left for your account. %sYou can do Deep Keyword Research and get up to 50 results on each research.", _SQ_PLUGIN_NAME_), '<strong>', $view->checkin->product_type, '</strong>', (int)$view->checkin->subscription_kr, '<br />'); ?>
                </div>
            <?php } elseif ($view->checkin->subscription_status == 'freemium') { ?>
                <div class="alert alert-warning text-center m-0 p-1 small">
                    <?php echo sprintf(__("%sResearch %s:%s You have %s researches left for your account. The research will return up to 10 results for each keyword. %sFor more Researches and up to 50 results per research, please upgrade your plan to %sBusiness Plan%s", _SQ_PLUGIN_NAME_), '<strong>', $view->checkin->product_type, '</strong>', (int)$view->checkin->subscription_kr, '<br />', '<a href="' . SQ_Classes_RemoteController::getMySquirrlyLink('plans') . '" target="_blank"><strong>', '</strong></a>'); ?>
                </div>
            <?php } ?>
        <?php } ?>
        ...
        <?php if ($page == 'sq_research' && SQ_Classes_Helpers_Tools::getValue('tab', '') == 'briefcase') { ?>
            <div class="alert alert-success text-center m-0 p-1 small">
                <?php echo sprintf(__("%sSquirrly Briefcase:%s Add unlimited keywords in your Squirrly Briefcase to optimize your posts and pages.", _SQ_PLUGIN_NAME_), '<strong>', '</strong>'); ?>
            </div>
        <?php } ?>
        <?php if ($page == 'sq_research' && SQ_Classes_Helpers_Tools::getValue('tab', '') == 'labels') { ?>
            <div class="alert alert-success text-center m-0 p-1 small">
                <?php echo sprintf(__("%sSquirrly Labels:%s Add unlimited Labels for the Squirrly Briefcase keywords to organize the keywords by your SEO strategy.", _SQ_PLUGIN_NAME_), '<strong>', '</strong>'); ?>
            </div>
        <?php } ?>
    ...
    <?php if ($page == 'sq_bulkseo') { ?>
        <div class="alert alert-success text-center m-0 p-1 small">
            <?php echo sprintf(__("%sBulk SEO Settings:%s This feature is included in all versions of Squirrly SEO for free.", _SQ_PLUGIN_NAME_), '<strong>', '</strong>'); ?>
        </div>
    <?php } ?>
