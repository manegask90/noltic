...
<td style="min-width: 380px;">
    ...
    <div class="sq_auditpages_url small"><?php echo '<a href="' . $view->auditpage->permalink . '"  class="text-link" rel="permalink" target="_blank">' . urldecode($view->auditpage->permalink) . '</a>' ?></div>
    <div class="sq_focuspages_lastaudited small my-1"><?php echo __('Last checked', _SQ_PLUGIN_NAME_) ?>:
                ...
                <li class="m-0 p-1 py-2">
                    <form method="post" class="p-0 m-0" onSubmit="return confirm('<?php echo __('Do you want to delete the Audit Page?',_SQ_ASSETS_URL_) ?>') ">
                        ...
                        <button type="submit" class="btn btn-sm bg-transparent p-0 m-0">
                            <?php echo __('Delete Page from Audit', _SQ_PLUGIN_NAME_) ?>
                        </button>
                    </form>
                </li>
                <li class="m-0 p-1 py-2">
