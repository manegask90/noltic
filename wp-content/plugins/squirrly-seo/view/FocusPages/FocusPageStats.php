                        ...
                        switch ($name) {
                            ...
                            case 'page_views':
                                if (!empty($values)) {
                                    ...
                                    foreach ($values as $date => $value) {
                                        $views[] = array(date('m/d/Y', strtotime($date)), $value);
                                    }
                                }
                                break;
                        }
                            ...
                            case 'ranking':
                                if (!empty($value)) {
                                    foreach ($value as $keyword => $increase) {
                                        $progress[] = sprintf(__('Rank increased %s positions for the keyword: %s', _SQ_PLUGIN_NAME_), '<strong>' . $increase . '</strong>', '<br /><strong>' . $keyword . '</strong>');
                                    }
                                }
                                break;
                            case 'time':
                                if ($value && $value > 60) {
                                    $progress[] = sprintf(__('Time on Page increased with %s minutes', _SQ_PLUGIN_NAME_), '<strong>' . number_format(($value / 60), 0, '', '') . '</strong>');
                                }
                                break;
                            case 'traffic':
                                if ($value) {
                                    $progress[] = sprintf(__('Page Traffic increased with %s visits', _SQ_PLUGIN_NAME_), '<strong>' . $value . '</strong>');
                                }
                                break;
                            case 'clicks':
                                if ($value) {
                                    foreach ($value as $keyword => $increase) {
                                        $progress[] = sprintf(__('Organic Clicks increased with %s for the keyword: %s', _SQ_PLUGIN_NAME_), '<strong>' . $increase . '</strong>', '<br /><strong>' . $keyword . '</strong>');
                                    }
                                }
                                break;
                            case 'authority':
                                if ($value) {
                                    $progress[] = sprintf(__('Page Authority increased with %s', _SQ_PLUGIN_NAME_), '<strong>' . $value . '</strong>');
                                }
                                break;
                            case 'social':
                                if ($value) {
                                    $progress[] = sprintf(__('You got %s Social Shares', _SQ_PLUGIN_NAME_), '<strong>' . $value . '</strong>');
                                }
                                break;
                            case 'seo':
                                if ($value) {
                                    foreach ($value as $seo => $time) {
                                        if ($seo == 'loading_time' && $time) {
                                            $progress[] = sprintf(__('Page loads with %ss faster', _SQ_PLUGIN_NAME_), '<strong>' . $time . '</strong>');
                                        }
                                    }
                                }
                                break;
                ...
                <div class="card-content overflow-hidden m-0">
                    <div class="media align-items-stretch">
                        ...
                        <div class="media-body p-3">
                            <h5><?php echo __('Chances of Ranking', _SQ_PLUGIN_NAME_) ?></h5>
                            <span class="small"><?php echo sprintf(__('the latest %s days evolution for this Focus Page', _SQ_PLUGIN_NAME_), $days_back) ?></span>
                ...
                <div class="card col-sm p-0 m-1 bg-white shadow-sm">
                    <div class="card-content  overflow-hidden m-0">
                        <div class="media align-items-stretch">
                            ...
                            <div class="media-body p-3">
                                <h5><?php echo __('Keyword Ranking', _SQ_PLUGIN_NAME_) ?></h5>
                                <span class="small"><?php echo sprintf(__('the latest %s days ranking for %s', _SQ_PLUGIN_NAME_), $days_back, '<strong>' . $focus_keyword . '</strong>') ?></span>
                                <div class="media-right py-3 media-middle ">
                                    <div class="col-sm-12 px-0">
                ...
                <div class="card col-sm p-0 m-1 bg-white shadow-sm">
                    <div class="card-content  overflow-hidden m-0">
                        <div class="media align-items-stretch">
                            ...
                            <div class="media-body p-3">
                                <h5><?php echo __('Page Traffic', _SQ_PLUGIN_NAME_) ?></h5>
                                <span class="small"><?php echo sprintf(__('the latest %s days page views', _SQ_PLUGIN_NAME_), $days_back) ?></span>
                                <div class="media-right py-3 media-middle ">
                                    <div class="col-sm-12 px-0">
